...
#if canImport(swiftandaluhFFI)
import swiftandaluhFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_swiftandaluh_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_swiftandaluh_rustbuffer_free(self, $0) }
    }
}
...
public func expensive(input: String) async  -> Response? {
    ...
    return try!  await withCheckedThrowingContinuation {
        continuation = $0
        try! rustCall() {
            uniffi_swiftandaluh_fn_func_expensive(
...
public func transform(input: String)  -> String? {
    return try!  FfiConverterOptionString.lift(
        try! rustCall() {
    uniffi_swiftandaluh_fn_func_transform(
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_swiftandaluh_uniffi_contract_version()
